...
/**
 * Serializes list of UserAccount instances.
 *
 * @author Vehbi Sinan Tunalioglu
 */
@Provider
@Produces(MediaType.APPLICATION_JSON)
public class UserAccountListWriter implements MessageBodyWriter<List<UserAccount>> {
    @Override
    ...
    @Override
    public long getSize(List<UserAccount> userAccountList, Class<?> aClass, Type type, Annotation[] annotations, MediaType mediaType) {
        return -1;
    }

    @Override
    public void writeTo(List<UserAccount> userAccountList, Class<?> aClass, Type type, Annotation[] annotations, MediaType mediaType, MultivaluedMap<String, Object> multivaluedMap, OutputStream outputStream) throws IOException, WebApplicationException {
        // Get the JSON generator:
        JsonGenerator gen = Json.createGenerator(outputStream);

        // Start writing the list:
        gen.writeStartArray();
